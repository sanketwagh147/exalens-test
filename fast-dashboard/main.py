...
@app.get("/")
async def index(request:Request, sensor_id:str, page:int =1,sensor_type:str="",start_date: str = "", end_date: str = "", ):
    ...
    if sensor_type:
        query["sensor_id"] = {"$regex": f"sensor_{sensor_type}_\\d+", "$options": "i"}
    if start_date:
    ...
    temperature_readings = await collection.find(query).sort("_id", -1).skip(skip_count).limit(readings_per_page).to_list(None)
    ...
    return templates.TemplateResponse("index.html", {"request": request, "readings": readings, "page": page, "total_pages": total_pages, "sensor_type": sensor_type or "All sensor reading", "sensor_types": SENSOR_TYPES, "current_sensor_id": sensor_id})
